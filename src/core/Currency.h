...
public:
  ...
  uint32_t upgradeHeight() const { return m_upgradeHeightV3; }
  ...
  uint32_t maxUpgradeDistance() const { return static_cast<uint64_t>(m_upgradeWindow); }
  uint32_t calculateUpgradeHeight(uint64_t voteCompleteHeight) const { return voteCompleteHeight + m_upgradeWindow; }
